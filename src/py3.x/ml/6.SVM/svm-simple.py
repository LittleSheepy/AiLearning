...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    ...
    # 矩阵转置 和 .T 一样的功能
    classMatrix = np.mat(classLabels)
    labelMat = classMatrix.transpose()
    ...
    while (iter < maxIter):
        ...
        for i in range(m):
            ...
            if ((labelMat[i]*Ei < -toler) and (alphas[i] < C)) or ((labelMat[i]*Ei > toler) and (alphas[i] > 0)):
                ...
                # 计算出一个新的alphas[j]值
                new_eta = labelMat[j]*(Ei - Ej)/eta
                alphas[j] -= new_eta
                # 并使用辅助函数，以及L和H对其进行调整
                new_alphas = clipAlpha(alphas[j], H, L)
                alphas[j] = new_alphas
